...
var csrf1 string = "" //     cookie里的bili_jct

func main() {
	for {
		danmu()
		zbj()
		time.Sleep(time.Second * 5) //大主播的话就调0  臭底边调10也行 反正没啥弹幕
...
func seqing(text string, uid int64, ts int64, nickname string) {
	...
	client := &http.Client{}
	var data = strings.NewReader(fmt.Sprintf("id=0&roomid=%v&tuid=%v&msg=%v&reason=%v&ts=%v&sign=&reason_id=2&token=&dm_type=0&csrf_token=%v&csrf=%v&visit_id=", room, uid, strData, seqin, ts, csrf1, csrf1))
...
func ruma(text string, uid int64, ts int64, nickname string) {
	...
	client := &http.Client{}
	var data = strings.NewReader(fmt.Sprintf("id=0&roomid=%v&tuid=%v&msg=%v&reason=%v&ts=%v&sign=&reason_id=4&token=&dm_type=0&csrf_token=%v&csrf=%v&visit_id=", room, uid, strData, seqin, ts, csrf1, csrf1))
	req, err := http.NewRequest("POST", "https://api.live.bilibili.com/xlive/web-ucenter/v1/dMReport/Report", data)
	if err != nil {
		log.Fatal(err)
	}
...
func zbj() {
	...
	client := &http.Client{}
	var data = strings.NewReader(fmt.Sprintf("room_id=%v&picUrl=&reason=%v&csrf_token=%v&csrf=%v&visit_id=", room, neirong, csrf1, csrf1))
